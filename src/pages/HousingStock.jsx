  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-gray-50">
      <div className="container mx-auto px-4 py-4">
        ...
        <div className={`${isMobile ? 'flex-col' : 'flex lg:flex-row'} gap-4 ${isMobile && !showFilters ? 'hidden' : 'flex'}`}>
          {/* Filters Panel */}
          <div className={`${isMobile ? 'w-full' : 'w-full lg:w-1/4'} space-y-4 ${isMobile && showFilters ? 'mb-4' : ''}`}>
            <Filters 
              filters={filters} 
            ...
            />
            {!isMobile && <Statistics stats={stats} />}
          </div>
        </div>

        {/* Map Panel - Always visible */}
        <div className={`w-full ${isMobile ? 'h-[60vh]' : 'lg:w-3/4 h-[calc(100vh-12rem)]'} ${isMobile ? 'ml-0' : 'lg:ml-auto'}`}>
          {loading && (
        ...
        {/* Stats panel for mobile - shown below map when filters are hidden */}
