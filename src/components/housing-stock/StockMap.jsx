...
const StockMap = ({ filters, colorScheme = 'buildingType', isMobile }) => {
  ...
  const getLegendItems = () => {
    const items = {
      buildingType: [
        ...
        { color: '#9467bd', label: 'สาธารณูปโภค สaธารณูปการ' },
    ...
    try {
      ...
      fetch('/data/bldg_pkn.geojson')
        ...
        .catch(error => {
          ...
          setMapInitialized(true); // Still mark as initialized to prevent retries
        });
  ...
  }, [isMobile, mapInitialized]);

  // Update filters when they change
  useEffect(() => {
    if (!mapRef.current || !buildingLayerRef.current || !mapInitialized) return;
  ...
  useEffect(() => {
    ...
    updateLegend();
  }, [colorScheme, mapInitialized]);

  if (!mapInitialized) {
    return (
      <div className="bg-white rounded-lg shadow-md overflow-hidden relative h-full flex items-center justify-center">
